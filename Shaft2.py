...
def user_input_features():
    with st.sidebar.expander("Dimensional Parameters"):
        ...
        f =  st.number_input('Fatigue Strength Fraction (f)', min_value=0.0, value=30.0, step=0.1, format="%.3f",
...
def perform_calculations(df):
    ...
    if results['Kf'] is not None and results['Section Modulus (mm³)'] > 0:
        ...
        results['σ_a (MPa)'] =  (results['M_B (N·m)'] / results['Section Modulus (mm³)'])* (results['Kf'])
