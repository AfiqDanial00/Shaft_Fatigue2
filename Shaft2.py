...
def perform_calculations(df):
    ...
    if Kf and Z > 0:
        ...
        # Finite life calculations (only if Se is available and stress exceeds endurance limit)
        if Se and sigma_a_prime and sigma_a_prime > Se and sigma_a_prime < vals['Sy']:
            # Calculate finite life parameters
...
def format_results(results):
    formatted = {k: ("N/A" if v is None else f"{v:.3f}") for k, v in results.items()}
    # Special formatting for cycles to show in thousands
    if 'Cycles to Failure N' in results and results['Cycles to Failure N'] is not None:
        formatted['Cycles to Failure N'] = f"{results['Cycles to Failure N']/1000:.1f} × 10³"
    return formatted
...
# Finite Life Calculations Section
st.subheader("Finite Life Calculations")
if calc_results.get('Cycles to Failure N'):
    st.latex(r'''
    \begin{aligned}
    a &= \frac{(f \cdot S_{ut})^2}{S_e} = \frac{(%s \cdot %s)^2}{%s} = %.1f \text{ MPa} \\
    b &= -\frac{1}{3}\log\left(\frac{f \cdot S_{ut}}{S_e}\right) = -\frac{1}{3}\log\left(\frac{%s \cdot %s}{%s}\right) = %.4f \\
    N &= \left(\frac{\sigma_{ar}}{a}\right)^{1/b} = \left(\frac{%.1f}{%.1f}\right)^{1/%.4f} = %.1f \times 10^3 \text{ cycles}
    \end{aligned}
    ''' % (
        input_df['f'].values[0], input_df['UTS'].values[0], formatted['Se'],
        calc_results['Finite Life a'],
        input_df['f'].values[0], input_df['UTS'].values[0], formatted['Se'],
        calc_results['Finite Life b'],
        calc_results['σa_prime'], calc_results['Finite Life a'], 
        calc_results['Finite Life b'],
        calc_results['Cycles to Failure N']/1000 if calc_results['Cycles to Failure N'] else 0
    ))
    
    st.table(pd.DataFrame({
        'Parameter': ['a', 'b', 'Cycles to Failure (N)'],
        'Value': [formatted.get('Finite Life a', 'N/A'), 
                 formatted.get('Finite Life b', 'N/A'),
                 formatted.get('Cycles to Failure N', 'N/A')],
        'Units': ['MPa', '-', 'cycles']
    }))
else:
