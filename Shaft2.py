...
if 'a' in calc_results and calc_results['a'] is not None:
    st.subheader("Fatigue Life Prediction")
    st.table(pd.DataFrame({
        'Parameter': ['a', 'b', 'N'],
        'Value': [formatted['a'], formatted['b'], formatted['N']],
        'Units': ['MPa', '-', 'cycles']
    }))
else:
    st.warning("Insufficient data to calculate fatigue life parameters")
